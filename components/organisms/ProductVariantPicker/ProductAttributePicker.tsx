...
const ProductAttributePicker = ({
  ...
  const allowedValues = useMemo(
    () =>
      allowedVariants?.flatMap((variant) =>
        variant.attributes
          .filter((attributeItem) => attributeItem.attribute.id === attribute.id)
          .flatMap((attributeItem) => attributeItem.values.map((valueItem) => valueItem.value)),
      ),
    [allowedVariants, attribute.id],
  );

  const attributeOptions: AttributeOption[] = values.map((value) => {
    return {
