...
const getTranslate = (side: "left" | "right") => (side === "left" ? "-100%" : "100%");
...
export const Lightbox = styled.div<IStyleProps>`
  display: flex;
  position: relative;
  width: ${({ position, theme: { modal } }) => lightboxHeight(modal.modalWidth)[position]};
...
export const Overlay = styled.div<IStyleProps>`
  ...
  background-color: ${({ transparent, theme }) => (transparent ? "" : theme.colors.overlay)};
  align-items: center;
