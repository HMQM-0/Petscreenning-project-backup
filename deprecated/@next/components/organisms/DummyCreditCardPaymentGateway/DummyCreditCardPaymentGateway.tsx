...
const DummyCreditCardPaymentGateway: React.FC<IProps> = ({
  config,
  processPayment,
  formRef,
  formId,
  ...
  const handleSubmit = async (formData: ICardInputs) => {
    ...
    if (payment?.token) {
      ...
      const errors = [
        {
          message:
            "Payment submission error. Dummy credit card gateway returned no token in payload.",
        },
      ];
  ...
  return (
    <S.Wrapper data-test="dummyCreditCardPaymentGateway">
      <CreditCardForm
        formRef={formRef}
        formId={formId}
...
export { DummyCreditCardPaymentGateway };
