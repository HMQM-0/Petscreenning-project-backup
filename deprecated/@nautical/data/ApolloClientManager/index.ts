...
import * as AffiliateMutations from "../../mutations/affiliates";
import {
...
import {
  ...
  UpdateCheckoutSellerShippingMethods,
  UpdateCheckoutSellerShippingMethodsVariables,
...
} from "../../mutations/gqlTypes/AffiliateCode";
...
export class ApolloClientManager {
  ...
  setSellerShippingMethods = async (
    ...
    try {
      const { data, errors } = await this.client.mutate<
        ...
        UpdateCheckoutSellerShippingMethodsVariables
      >({
        mutation: CheckoutMutations.updateCheckoutSellerShippingMethodsMutation,
        variables: {
          checkoutId,
          seller,
      ...
      if (data?.checkoutSellerShippingMethodsUpdate?.checkout) {
        return {
          data: this.constructCheckoutModel(
            data.checkoutSellerShippingMethodsUpdate.checkout
          ),
        };
      }
      return {};
  ...
  useAffiliateCode = async (code: string) => {
    try {
      const { data, errors } = await this.client.mutate<
        AffiliateCodeUse,
        AffiliateCodeUseVariables
      >({
        mutation: AffiliateMutations.useAffiliateCodeMutation,
        variables: { code },
      });
      ...
      if (data?.affiliateCodeUse?.errors?.length) {
        return {
          error: data?.affiliateCodeUse?.errors,
        };
      }

      if (data?.affiliateCodeUse?.affiliateCodes) {
        return {
          data: data.affiliateCodeUse.affiliateCodes,
        };
      }
  ...
  completeCheckout = async ({
    ...
    try {
      ...
      const cleanedVolumeDiscountsBySeller = volumeDiscountsBySeller?.map(
        (v) => {
          return {
            seller: v.seller,
            volumeDiscount: {
  ...
  private constructCheckoutModel = ({
    ...
    availableShippingMethods: availableShippingMethods
      ...
      : [],
    availableShippingMethodsBySeller: availableShippingMethodsBySeller
      ? availableShippingMethodsBySeller
      : [],
    billingAddress,
    email,
